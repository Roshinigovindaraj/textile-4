    ...
    <footer className="bg-dark-charcoal text-gray-300">
      {showNewsletter && (
        <div className="border-b border-white/10 py-10 px-4">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <p className="text-xs tracking-[0.2em] text-rose uppercase mb-1">
                JOIN THE VĀYU CIRCLE
              </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10">
        ...
        <div className="lg:col-span-2">
          <div className="mb-4">
            <span className="font-playfair text-2xl font-bold text-white tracking-widest block">
              VĀYU
            </span>
            <span className="text-[9px] tracking-[0.25em] text-gold font-semibold">
              SILK SAREES
            </span>
          </div>

          <p className="text-sm leading-relaxed text-gray-400 mb-6 max-w-xs">
            Celebrating India's weaving heritage through handcrafted silk
            sarees, made with love by master artisans across the country.
          </p>
          ...
          <div className="flex gap-3">
            {['instagram', 'facebook-f', 'pinterest-p', 'youtube'].map((s) => (
              <a
                ...
                className="w-9 h-9 rounded-full border border-white/20 flex items-center justify-center text-sm text-gray-300 hover:bg-rose hover:border-rose hover:text-white transition-all duration-300"
              >
        ...
        {/* Quick Links */}
        <div>
          <h4 className="text-xs font-semibold tracking-[0.2em] text-white mb-5">
            QUICK LINKS
          </h4>
          ...
          <ul className="space-y-3">
            {[
              ...
              <li key={i}>
                <Link
                  to={to}
                  className="text-sm text-gray-400 hover:text-rose transition-colors duration-200"
                >
                  {label}
                </Link>
              </li>
            ...
            {[
              {
                ...
                text: '12, Weavers Lane, Varanasi, Uttar Pradesh — 221001',
              },
              {
                icon: 'fa-phone-alt',
                text: '+91 98765 43210',
              ...
              {
                icon: 'fa-envelope',
                text: 'hello@vayusilk.com',
              },
              {
                icon: 'fa-clock',
                text: 'Mon – Sat: 10am – 7pm IST',
              },
            ].map((item, i) => (
              <li key={i} className="flex gap-3 text-sm text-gray-400">
                <i
                  className={`fas ${item.icon} text-gold mt-0.5 shrink-0`}
                ></i>
                <span>{item.text}</span>
              </li>
            ))}
